  ...
  <body bgcolor="darksalmon"><!--Add BG Color to make footer cover the area-->
    <div class="container">
      ...
      <div class="recipe-container">
            <?php
            $sql = "SELECT * FROM recipes,likes WHERE userid='$userid' AND id=recipeid";
            $result = mysqli_query($db, $sql);
            if($result){
                if(mysqli_num_rows($result) > 0){
                    echo "<h3 class='subtitle center pulse'>Liked Recipes</h3>";
                    ...
                    while($row = mysqli_fetch_assoc($result)){
                        ...
                        echo "<a href='viewRecipe.php?id=$id&back=viewUserLikes.php&args=id,$userid,back,$back,args,$newArgs'>View Recipe</a>";
                        if($isSuperUser){
                            echo "<a class='delete' href='removeLike.php?userid=$userid&recipeid=$id&back=viewUserLikes.php&args=id,$userid,back,$back,args,$newArgs'>Remove Like</a>";
                        }
                        echo "</div>";
                    }
                    echo "</div>";
                }else {
                    echo "<h3 class='subtitle center pulse'>No Liked Recipes</h3>";
                }
            }
            ?>
        </div>
